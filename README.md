...
## Bonus: personal global default

The per-project approach above is what teams and CI need. **Solo scratch-coding** is different, when working in a throwaway directory under `~/projects/` or `~/code/`, copying a whole stack folder is overkill. A single [.prettierrc.json](.prettierrc.json) in a location Prettier already searches gives the same formatting everywhere with zero per-repo setup.

Two viable locations (Prettier 3+):

- **`~/.config/prettier/config.json`**: the canonical XDG fallback. Used when no project-level config is found.

- **`~/<workroot>/.prettierrc.json`**: a parent directory of all your projects. Picked up only when the inner project has no `.git`, no `package.json` and no closer config above it.

Per-project configs **always win**, so this never interferes with a stack copied from this repo.
